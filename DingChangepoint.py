...
def participant_worker(pId, datasetName, rotation, impFlag, human_signed_full, rot_full, baseline_length, washout_length, debug, save_dir, n_participants):
    rows = []
    if debug: print(f"Generating plot for {datasetName}, p{pId}...")
    human_aims_signed = human_signed_full[pId]
    n_trials = len(human_aims_signed)
    if n_trials == 0:
        if debug: print(f" No human aims data for p{pId}; skipping.")
        return rows
    human_imps_signed = np.array([])                
    rot_part = rot_full[pId]
                                      
    non_zero_rots = rot_part[rot_part != 0]
    if len(non_zero_rots) > 0:
        rotation = abs(float(np.unique(non_zero_rots)[0]))
    else:
        rotation = 0.0
    ...
    if min_t <= max_t:
        ...
        if n_base_full < 2:
            ...
            post_aims = aim[:max_t + 1]                 
            cp_rels = ruptures_changepoint(post_aims, bl=bl, pen=0.2, min_size=1, debug=debug)
            if cp_rels:
                aha_trials = [cp_rels[0]['position']]
    ...
    for i, trial in enumerate(aha_trials):
        ...
        ax.text(x=x_text, y=y_text, s=s, ha=ha, va='bottom', fontsize=80, color=color, weight='bold', clip_on=False)
    ...
    if has_washout:
        ...
        if intervals == int(intervals) and int(intervals) % 2 == 1:
            current_gcd = step
...
def generate_per_participant_plots(csv_path, save_dir=plot_dir, debug=False):
    """
    Generate per-participant plots for Ding data using Ruptures changepoint detection.
    ...
    reformatted_path = 'ding_reformatted.csv'
    df_ding.to_csv(reformatted_path, index=False)
    print(f"Reformatted Ding data saved to: {reformatted_path}")

    participant_ids = sorted(df_ding['participant_idx'].unique())
    n_parts = len(participant_ids)
    ding_n_trials = 132         

    human_signed_full = np.full((n_parts, ding_n_trials), np.nan)
    rot_full = np.full((n_parts, ding_n_trials), np.nan)

    for i, part_id in enumerate(participant_ids):
        part_df = df_ding[df_ding['participant_idx'] == part_id].sort_values('trial_number')
        trial_indices = part_df['trial_number'].values - 1
        signed_aims = part_df['aim_signed'].values
        rots = part_df['Rotation'].values
        human_signed_full[i, trial_indices] = signed_aims
        rot_full[i, trial_indices] = rots

                                                           
    datasetName = 'DingPooled'
    impFlag = False
    baseline_length = 24
    washout_length = 12
    ...
    with mp.Pool() as pool:
        worker_args = [(pId, datasetName, 0, impFlag, human_signed_full, rot_full, baseline_length, washout_length, debug, save_dir, n_parts) for pId in range(n_parts)]
    ...
    csv_path_out = 'ding_changepoints.csv'
    changepoints_df.to_csv(csv_path_out, index=False)
    print(f"All per-participant plots generated and saved to {plot_dir}/")
    print(f"Changepoints saved to: {csv_path_out}")

if __name__ == '__main__':
    if len(sys.argv) < 2:
        raise ValueError("Please provide the path to the CSV file containing the Ding data.")
    csv_path = sys.argv[1]
    generate_per_participant_plots(csv_path, debug=False)