...
def computeNegLl(params, allAims, mask, trials, isRotation, rotation, targets, uniqueThetas):
    ...
    for idx in range(numTrials):
        ...
        predictivePGlobal = 0.0
        for k in range(runLimit):
            predictivePGlobal += pKArr[k] * pGlobalRun[k]
        expectedPert = pPNorm * (predictivePGlobal * globalSign * muAct + (1.0 - predictivePGlobal) * (2.0 * pPos - 1.0) * muAct)
        plannedOffset = -expectedPert
        if deltaObs == 0.0:
            plannedOffset = 0.0
        snappedOffset = snapExplicit(plannedOffset)
        plannedAimAngle = wrapAngle(currentTarget + snappedOffset)
        ...
        logMargLikP = logAddExp(safeLog(predictivePGlobal) + logMargLikG, safeLog(1.0 - predictivePGlobal) + logMargLikD)
        ...
        for k in range(runLimit):
            ...
            explicitK = -globalSign * muRunPrev[k]
            snappedE = snapExplicit(explicitK)
            meanK = implicitCompensation + snappedE
            logComp = logGaussianPdf(aim, meanK, math.sqrt(compVar))
            logPdfMargG = logAddExp(logPdfMargG, logR[t-1, k, 1] + logComp)
        logPdfMargG -= logSumPrevP
        logPdfMargDPos = LOGZERO
        for k in range(runLimit):
            compVar = varMuArrPrev[k] + sigmaAim2
            explicitK = -muRunPrev[k]
            snappedE = snapExplicit(explicitK)
            meanK = implicitCompensation + snappedE
            logComp = logGaussianPdf(aim, meanK, math.sqrt(compVar))
        ...
        logPdfP = logAddExp(safeLog(predictivePGlobal) + logPdfMargG, safeLog(1.0 - predictivePGlobal) + logPdfMargD)
        logPdf = logAddExp(safeLog(pS0Norm) + logPdfS0, safeLog(pPNorm) + logPdfP)
        if deltaObs == 0.0:
            logPdf = safeLog(pS0Norm) + logPdfS0
        if mask[trial]:
            logLikelihood += logPdf
        if absEffective > 0.0:
            globalSign = signEffective
        postpGlobal = math.exp(safeLog(predictivePGlobal) + logMargLikG - logMargLikP) if safeLog(predictivePGlobal) + logMargLikG != LOGZERO else 0.0
...
class BayesianStepper:
    ...
    def snapExplicit(self, x, grid=5.625):
        return round(x / grid) * grid
    ...
    def getPredictive(self, trialNum, currentTarget, deltaObs=0.0):
        ...
        varAct = max(varActTemp - muAct**2, 0.0)
        predictivePGlobal = np.sum(pKArrP * self.pGlobalRun[:runLimit])
        expectedPert = pP * (predictivePGlobal * self.globalSign * muAct + (1 - predictivePGlobal) * (2 * pPos - 1) * muAct)
        plannedOffset = -expectedPert
        if deltaObs == 0.0:
            plannedOffset = 0.0
        snappedOffset = self.snapExplicit(plannedOffset)
        plannedAimAngle = self.wrapAngle(currentTarget + snappedOffset)
        ...
        return {
            'pS0': pS0,
            'pP': pP,
            'pG': pP * predictivePGlobal,
            'pD': pP * (1 - predictivePGlobal),
            'var0': var0,
            ...
            'runLimit': runLimit,
            'predictivePGlobal': predictivePGlobal,
            'pPos': pPos,
    ...
    def updatePosteriors(self, trialNum, deltaObs, currentTarget):
        ...
        predDict = self.getPredictive(trialNum, currentTarget, deltaObs)
        ...
        pS0 = predDict['pS0']
        pP = predDict['pP']
        predictivePGlobal = predDict['predictivePGlobal']
        pPos = predDict['pPos']
        logLik0 = logGaussianPdf(effectiveDeltaObs, 0.0, effectiveSigma)
        ...
        logMargLikP = self._logAddExp(np.log(predictivePGlobal) + logMargLikG, np.log(1 - predictivePGlobal) + logMargLikD)
        ...
        self.postpGlobal = np.exp(np.log(predictivePGlobal) + logMargLikG - logMargLikP) if not np.isinf(np.log(predictivePGlobal) + logMargLikG) else 0.0
    ...
    def expectedMove(self, trialNum, currentTarget, deltaObs=0.0):
        if deltaObs == 0.0:
            return 0.0
        predDict = self.getPredictive(trialNum, currentTarget, deltaObs)
        expectedPert = predDict['pP'] * (predDict['predictivePGlobal'] * predDict['globalSign'] * predDict['muAct'] + (1 - predDict['predictivePGlobal']) * (2 * predDict['pPos'] - 1) * predDict['muAct'])
        expectedAim = -expectedPert
        expectedAim = self.wrapAngle(expectedAim)
        expectedAim = self.snapExplicit(expectedAim)
        return expectedAim
def plotCombined(modelExplicit, modelImps, mOutsSingle, humanExplicit, humanImps, allAims, trials, number, plotIdentifier, fittedParams, targets, compMags, numSamples=100):
    ...
    for trial in trials:
        ...
        currentTarget = targets[trial]
        predDict = stepper.getPredictive(trial, currentTarget, deltaObs)
        pS0 = predDict['pS0']
        pG = predDict['pG']
        pD = predDict['pD']
        ...
        if deltaObs == 0.0:
            numS0 = numSamples
            scaleS0 = sigmaMotor
            samp = norm.rvs(loc=implicit, scale=scaleS0, size=numS0)
            ...
            if numS0 > 0:
                locS0 = implicit
                scaleS0 = sigmaMotor
                samp = norm.rvs(loc=locS0, scale=scaleS0, size=numS0)
                samp = np.array([stepper.wrapAngle(s) for s in samp])
                aims.extend(samp)
                componentsList.extend(['S0'] * numS0)
                trialsList.extend([trial] * numS0)
            if numG > 0:
                ...
                explicitG = -globalSign * muRun[ks]
                snappedExplicitG = np.round(explicitG / 5.625) * 5.625
                locsG = implicit + snappedExplicitG
            ...
            if numD > 0:
                ...
                explicitD = -signs * muRun[ks]
                snappedExplicitD = np.round(explicitD / 5.625) * 5.625
                locsD = implicit + snappedExplicitD
...
def plotPointEstimates(modelExplicit, mOutsSingle, modelImps, humanExplicit, humanImps, trials, number, plotIdentifier, fittedParams, targets, compMags):
    ...
    for trial in trials:
        deltaObs = compMags[trial]
        currentTarget = targets[trial]
        expectedAim = stepper.expectedMove(trial, currentTarget, deltaObs)
...
def plotSignFlipErrorProbability(dataList, fittedParamsList, plotIdentifier):
    ...
    for i, data in enumerate(dataList):
        ...
        for trial in trials:
            ...
            predDict = stepper.getPredictive(trial, currentTarget, deltaObs)
            pD = predDict['pP'] * (1 - predDict['predictivePGlobal'])
            ...
            modelAim = stepper.expectedMove(trial, currentTarget, deltaObs)
    ...
    def fitRot(self, numCores=multiprocessing.cpu_count()//2):
        ...
        for i in range(N):
            ...
            for trial in trials:
                currentTarget = targets[trial]
                deltaObs = conVal if phases[trial].lower() == 'rotation' else 0.0
                predDict = stepperSingle.getPredictive(trial, currentTarget, deltaObs)
                expectedExplicit = stepperSingle.expectedMove(trial, currentTarget, deltaObs)
                implicit = predDict['implicitCompensation']
            ...
            compMagsForViolin = [conVal if phases[j].lower() == 'rotation' else 0.0 for j in range(len(phases))]
            ...
            stepperSingleForEse = BayesianStepper(sigmaMotor, conVal, hazard, pS0, visCoeff, pGlobal, a, b, sigmaGen, uniqueTargets)
            sigmaAim2ForEse = sigmaMotor**2
            for trial in trials:
                if not mask[trial]:
                    continue
                currentTarget = targets[trial]
                deltaObs = compMagsForViolin[trial]
                predDict = stepperSingleForEse.getPredictive(trial, currentTarget, deltaObs)
                pS0 = predDict['pS0']
                pG = predDict['pG']
                pD = predDict['pD']
                pKArr = predDict['pKArr']
                ...
                implicit = predDict['implicitCompensation']
                if deltaObs == 0.0:
                    ese[trial] = pS0 * ((yT - implicit)**2 + sigmaAim2ForEse)
                else:
                    eseS0 = pS0 * ((yT - implicit)**2 + sigmaAim2ForEse)
                    snappedG = np.round(-globalSign * muRun[:runLimit] / 5.625) * 5.625
                    gTerm = ((yT - (implicit + snappedG))**2 + sigmaAim2ForEse + varMuArr[:runLimit])
                    eseG = pG * np.sum(pKArr[:runLimit] * gTerm)
                    snappedPos = np.round(-muRun[:runLimit] / 5.625) * 5.625
                    snappedNeg = np.round(muRun[:runLimit] / 5.625) * 5.625
                    posTerm = ((yT - (implicit + snappedPos))**2 + sigmaAim2ForEse + varMuArr[:runLimit])
                    negTerm = ((yT - (implicit + snappedNeg))**2 + sigmaAim2ForEse + varMuArr[:runLimit])
                    weighted = pPos * posTerm + (1 - pPos) * negTerm
                    eseD = pD * np.sum(pKArr[:runLimit] * weighted)
                    ese[trial] = eseS0 + eseG + eseD
                stepperSingleForEse.updatePosteriors(trial, deltaObs, currentTarget)
            numSamples = np.sum(mask)
