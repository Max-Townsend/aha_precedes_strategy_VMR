...
@njit(cache=True)
def computeNegLl(params, aims, rots, imps, targetPositions,
                 use_implicit=False, a=0.0, b=0.0, sigma_gen=0.0):
    noise = params[0]
    kappa = params[1]
...
def _fitWorker(args):
    pp, i, dat, fitPhase, imp, flipRot, bounds, initial, a, b, sigma_gen = args
    ...
    def localFitPp(u):
        ...
        return computeNegLl(p, aims, rots, imps, targetPositions, imp, a, b, sigma_gen)
    ...
    for i in range(n_starts):
        ...
        x0 = np.maximum(np.minimum(x0, ub), lb)
        ...
        clipped_x = np.maximum(np.minimum(res.x, ub), lb)
        candidates.append((res.fun, clipped_x))
    ...
    for t in range(n):
        ...
        max_q_mag = np.max(Q_mag)
        exp_q_mag = np.exp(beta * (Q_mag - max_q_mag))
        sum_exp_mag = np.sum(exp_q_mag)
        probs_mag = exp_q_mag / sum_exp_mag if sum_exp_mag > 0 else np.full(n_mag, 1.0 / n_mag)
                                                           
        max_q_dir = np.max(Q_dir)
        exp_q_dir = np.exp(beta_dir * (Q_dir - max_q_dir))
        sum_exp_dir = np.sum(exp_q_dir)
        probs_dir = exp_q_dir / sum_exp_dir if sum_exp_dir > 0 else np.array([0.5, 0.5])
    ...
    full_x = list(best_res.x) + [a, b, sigma_gen] if imp else list(best_res.x)
...
def _plotWorker(args):
    (i, pi_preds, model_predictive_pis, model_predictive_policies, valid_trials, human_explicits, human_implicits, all_model_implicits, xs_i, participantNums_i, temp_dir, model_predictive_dirs) = args
    import matplotlib.pyplot as plt
    from matplotlib.lines import Line2D
    from matplotlib.patches import Patch
    from scipy.stats import norm
    ...
    for t in range(n_trials):
        ...
        for k in range(num_aim_bins):
            ...
            i1 = norm.cdf(high, action_centers + implicit_comp_t, noise) - norm.cdf(low, action_centers + implicit_comp_t, noise)
            i2 = norm.cdf(high + 360, action_centers + implicit_comp_t, noise) - norm.cdf(low + 360, action_centers + implicit_comp_t, noise)
            i3 = norm.cdf(high - 360, action_centers + implicit_comp_t, noise) - norm.cdf(low - 360, action_centers + implicit_comp_t, noise)
            em1_contribs = i1 + i2 + i3
            em1 = np.dot(full_policy, em1_contribs)
            prob_k = pi_pred[0] * em0 + pi_pred[1] * em1
...
class FitShell:
    def __init__(self, df='none', conVal='none', condition='none',
        ...
        self.method = method
        self.a = a if imp else 0.0
        self.b = b if imp else 0.0
        self.sigma_gen = sigma_gen if imp else 0.0
    def fitRot(self, lrUb=1):
        ...
        argsList = [(pp, i, self.dat, self.fitPhase, self.imp,
                     self.flipRot, bounds, initial, self.a, self.b, self.sigma_gen)
    ...
    def fitPp(self, params):
        ...
        if self.imp:
            ...
            imps = np.full_like(aims, 0.0)
        return computeNegLl(params, aims, rots, imps, targetPositions,
                            self.imp, self.a, self.b, self.sigma_gen)
