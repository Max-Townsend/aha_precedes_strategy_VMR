...
@njit
def computeNegLl(params, aims, rots, imps, targets, lr2, retain2, sigma2):
    lr1, retain1, noise = params
    lr1 += lr2
    lr1 = min(1, lr1)
    logLikelihood = 0.0
    n = len(aims)
...
def _fitWorker(args):
    pp, i, dat, fitPhase, imp, flipRot, method, bounds, initial, lr2, retain2, sigma2 = args
    ...
    def localFitPp_orig(params):
        return computeNegLl(params, aims, rots, imps, targets, lr2, retain2, sigma2)
    def localFitPp(u):
        p = transform(u, bounds)
        return computeNegLl(p, aims, rots, imps, targets, lr2, retain2, sigma2)
    n_params = len(bounds)
    best_fun = np.inf
    n_starts = 500                                                              
...
class FitShell:
    def __init__(self, df='none', conVal='none', condition='none', fitLength=320, fitPhase=None, imp=True,
                 rmse=False, flipRot=False, method='CMA', lr2=0.005, retain2=0.99, sigma2=22.):
        self.conVal = conVal
    ...
    def fitRot(self, lrUB=1):
        ...
        self.targetPositions = []
        self.rotations = []
        bounds = [(0,1), (.8,1.), (1,100.)]
        initial = [.8, 0.99, 5.]
        argsList = [(pp, i, self.dat, self.fitPhase, self.imp, self.flipRot, self.method,
                     bounds, initial, self.lr2, self.retain2, self.sigma2)
        ...
        for i, res in enumerate(results):
            ...
            if False:
                ...
                filename = f'tempFigures/participant_{pp}_plot.png'
